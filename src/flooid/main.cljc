(ns flooid.main
  "Kotodama WASM entrypoint for the Flooid clean-room actor (L4) — Clojure port.
  ...
  facts (`flooid.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "flooid")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"    :plural "products"    :id-prefix "flooid_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "flooid_cus"
    :fields [:email :firstName :lastName :phone]         :required [:email :firstName]
    :coerce {}                                 :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "flooid_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"flooid.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "flooid-compat" :tier tier :entities entities} 200])
...
(defn start! [] :flooid-compat/ready)
